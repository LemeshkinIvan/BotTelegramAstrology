...
@bot.message_handler(commands=['bye']) #for fun and practice done!
def goodbye(message):
...
# part for adding users category

#in date bass and connecton
...
#if message.text == '/psychologic':
#    for i in range(1):
#        bot.send_message(message.chat.id, random.choice(psychologic_week))
#        time.sleep(6)
#        if message.text == 'shut up!':  # не читает условие
#            bot.send_message(message.chat.id, 'ты остановил текущую категорию')
#            break
